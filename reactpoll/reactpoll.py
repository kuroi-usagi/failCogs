...
class ReactPoll:
    ...
    async def rpoll(self, ctx, *text):
        ...
        if not self.getPollByChannel(message):
            ...
            if p.valid:
                ...
                await self.bot.say("`[p]rpoll Frage;Antwort1;Antwort2...;t=60`")
        else:
            await self.bot.say("Eine Umfrage läuft bereits in diesem Channel.")
    ...
    async def endpoll(self, message):
        if self.getPollByChannel(message):
            ...
            await self.bot.say("Zur Zeit gibt es keine Umfrage in diesem Channel.")
...
class NewReactPoll():
    ...
    def __init__(self, message, text, main):
        ...
        for answer in msg:  # {id : {answer, votes}}
            ...
            self.answers[i] = {"ANTWORT": answer, "VOTES": 0}
            i += 1
        self.message = None
    ...
    async def start(self):
        msg = "**UMFRAGE GESTARTET!**\n\n{}\n\n".format(self.question)
        for id, data in self.answers.items():
            msg += "{}\n".format(data["ANSWER"])
        msg += ("\nKlicke auf die Nummer für deine Antwort!"
                "\nUmfrage endet in {} Sekunden.".format(self.duration))
